...
// cSpell:word Enmity, ResetEnmity, ResetEnmityAll
...
namespace ResetEnmityCommand
{
  public class Plugin : IDalamudPlugin
  {
    ...
    */
    ...
    /// <summary>
    /// A boolean defining if the signature had been registered.
    /// </summary>
    ...
    private delegate long ExecuteCommandDelegate(uint id, ulong a1, int a2, int a3, int a4);
    ...
    private static long ExecuteCommandDetour(uint trigger, ulong a1, int a2, int a3, int a4)
    {
      _instance.Log.Debug($"trigger: {trigger}, a1: {a1:X}, a2: {a2:X}, a3: {a3:X}, a4: {a4:X}");
    ...
    {
      _instance = this;
      this.ExecuteCommand = RegisterSignature();
    }

    private ExecuteCommandDelegate? RegisterSignature()
    {
      try
      {
        unsafe
        {
          ...
          nint scanText = this.SigScanner.ScanText("E8 ?? ?? ?? ?? 8D 43 0A");
          /// Adds the memory delegate to execute the function at the pointer.
          var _executeCommand = Marshal.GetDelegateForFunctionPointer<ExecuteCommandDelegate>(scanText);
          /// Logs the execution of the reset striking dummy enmity command to the debug log.
          this.Log.Debug($"{nameof(this.ExecuteCommand)} +{scanText - Process.GetCurrentProcess().MainModule!.BaseAddress:X}");
...
#if DEBUG
          ExecuteCommandHook = GameInteropProvider.HookFromAddress<ExecuteCommandDelegate>(this.SigScanner.ScanText("E8 ?? ?? ?? ?? 8D 43 0A"), ExecuteCommandDetour);
      ...
      {
        this.Log.Error(exception, "Failed to register signature for resetting enmity on striking dummies");
    ...
    private void ResetEnmity(ulong objectId)
    {
      if (!this.Registered || this.ExecuteCommand is null) {
        return;
      }
      Log.Information($"Resetting enmity {objectId:X}");
      long success = this.ExecuteCommand(0x13f, objectId, 0, 0, 0);
    ...
    private void ResetTarget(string s, string s1)
    {
      ...
      if (target is ICharacter { NameId: 541 })
      {
        /// Reset the enmity of the target via it's objectId.
        ResetEnmity(target.GameObjectId);
      }
    }

    /// <summary>
    /// The reset enmity of all aggro-ed striking dummies.
    /// </summary>
    /// <param name="s">The command executed. (Unused)</param>
    /// <param name="s1">The parameters. (Unused)</param>
